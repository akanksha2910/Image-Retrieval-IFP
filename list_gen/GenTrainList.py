...
import random
label=0
fimgtest = open('imgtestc5.txt','w')
for key in imgtest:
  lenimgkey=len(imgtest[key])
  mylist=imgtest[key]
  if lenimgkey>8000:
    rand_smpl = [ mylist[i] for i in sorted(random.sample(xrange(lenimgkey), lenimgkey/8)) ]
  elif lenimgkey>3000:#1378*0.15:
    rand_smpl = [ mylist[i] for i in sorted(random.sample(xrange(lenimgkey), lenimgkey/6)) ]
  elif lenimgkey>206.7:
    rand_smpl = [ mylist[i] for i in sorted(random.sample(xrange(lenimgkey), lenimgkey/3)) ]
  elif lenimgkey>13.6:
    rand_smpl = [ mylist[i] for i in sorted(random.sample(xrange(lenimgkey), lenimgkey/2)) ] 
  else:
    rand_smpl=mylist
