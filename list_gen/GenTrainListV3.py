...
for key in subcatMap:
  ...
  if len(a)>9:
    ppp=2.0/len(a)
    imgtrain[key],imgtest[key]=train_test_split(a,test_size=ppp, random_state=42)
...
#%%
label=0
fimgtrain = open('imgtrainc500thrhd.txt','w')
for key in imgtrain:
...
#%%
label=0
fimgtrain = open('fimgtrainc10.txt','w')
for key in imgtrain:
  for imgid in imgtrain[key]:
    fimgtrain.write(imgid+'.jpg '+str(label)+'\n')
  label=label+1 # python will convert \n to os.linesep
...
#%%
label=0
fimgtest = open('fimgtestcp2.txt','w')
for key in imgtest:
